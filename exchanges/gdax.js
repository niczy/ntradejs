...
var Trader = function(config) {
  ...
  if (_.isObject(gdaxConfig)) {
    ...
    this.post_only = (typeof gdaxConfig.post_only !== 'undefined')
                         ? gdaxConfig.post_only
                         : true;
  }
  ...
  var result = function(err, response, data) {
    ...
    if (_.has(data, 'message')) {
      if (data.message === 'Invalid API Key' ||
          data.message === 'Invalid Passphrase')
    ...
    if (err) {
      if (err.message === 'NotFound') {
        ...
        return callback(undefined,
                        {price : 0, amount : 0, date : moment.unix(0)});
      }
...
Trader.prototype.getTrades = function(since, callback, descending) {
  ...
  var process = function(err, response, data) {
    ...
    if (this.scanback) {
      ...
      if (!this.scanbackTid) {
        ...
        if (moment.utc(last.time) < moment.utc(since)) {
          ...
          this.gdax_public.getProductTrades({
            after : last.trade_id - (batchSize * lastScan),
            limit : batchSize
          },
                                            process);
          lastScan++;
      ...
      if (this.scanbackTid) {
        // if scanbackTid is set we need to move forward
        // again
        log.debug('Backwards: ' + last.time + ' (' + last.trade_id + ') to ' +
                  first.time + ' (' + first.trade_id + ')');
        ...
        if (this.import) {
          ...
          if (this.scanbackTid != first.trade_id) {
            this.scanbackTid = first.trade_id;
            this.gdax_public.getProductTrades(
                {after : this.scanbackTid + batchSize + 1, limit : batchSize},
                process);
          } else {
            this.scanback = false;
            this.scanbackTid = 0;
            if (!this.import) {
              log.debug('Scan finished: data found:' +
                        this.scanbackResults.length);
...
Trader.prototype.getMaxDecimalsNumber = function(number, decimalLimit = 8) {
  ...
  // The ^-?\d*\. strips off any sign, integer portion,
  // and decimal point leaving only the decimal
  // fraction. The 0+$ strips off any trailing zeroes.
  var decimalCount =
      ((+decimalNumber).toString()).replace(/^-?\d*\.?|0+$/g, '').length;
  ...
  return decimalCount <= decimalLimit
             ...
             : (Math.floor(decimalNumber * decimalMultiplier) /
                decimalMultiplier)
