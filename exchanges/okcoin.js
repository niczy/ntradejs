...
var Trader = function(config) {
  _.bindAll(this);
  const okCoinConfig = config.okcoin;
  if (_.isObject(okCoinConfig)) {
    this.key = okCoinConfig.key;
    this.secret = okCoinConfig.secret;
  }
...
Trader.prototype.getPortfolio = function(callback) {
  var calculate = function(err, data) {
    if (err) {
      ...
      log.debug('Error in OkCoin#getPortfolio', err);
...
Trader.prototype.getTicker = function(callback) {
  ...
  var process = function(err, data) {
    ...
    var ticker = _.extend(data.ticker,
                          {bid : +data.ticker.sell, ask : +data.ticker.buy});
...
// are the "maker fee" of 0.1%.  It does not take into account
// volume discounts.
...
Trader.prototype.getTrades = function(since, callback, descending) {
  ...
  this.okcoin.getTrades(function(err, data) {
    ...
    var trades = _.map(data, function(trade) {
      return {
        price: +trade.price, amount: +trade.amount, tid: +trade.tid,
            date: trade.date
      }
    });
