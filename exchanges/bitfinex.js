...
var Trader =
  ...
  if (_.isObject(bfConfig)) {
    this.key = bfConfig.key;
    this.secret = bfConfig.secret;
  }
  this.name = 'Bitfinex';
  this.balance;
  this.price;
...
Trader.prototype.getDepth = function(callback) {
  var process = function(err, data, body) {
    data.asks = _.map(data.asks, (item) => [+item['price'], +item['amount']]);
    data.bids = _.map(data.bids, (item) => [+item['price'], +item['amount']]);
    callback(err, data);
  };
  this.bitfinex.orderbook(this.pair, {'limit_bids' : 10, 'limit_asks' : 10},
                          process);
};
...
Trader.prototype.getPortfolio =
  ...
  this.bitfinex.wallet_balances(function(err, data, body) {
    ...
    const currency =
        _.find(data, c => c.currency.toUpperCase() === this.currency);

    let assetAmount, currencyAmount;

    if (_.isObject(asset) && _.isNumber(+asset.available) &&
        !_.isNaN(+asset.available))
    ...
    if (_.isObject(currency) && _.isNumber(+currency.available) &&
        !_.isNaN(+currency.available))
  ...
  this.bitfinex.new_order(this.pair, amount + '', price + '',
                          this.name.toLowerCase(), type, 'exchange limit',
  ...
  this.bitfinex.trades(path, function(err, data) {
    ...
    var trades = _.map(data, function(trade) {
      return {
        tid: trade.tid, date: trade.timestamp, price: +trade.price,
            amount: +trade.amount
      }
    });
  ...
  return {
    ...
    assets : [
      'BTC', 'LTC', 'ETH', 'SAN', 'IOT', 'BCH', 'OMG', 'XMR', 'DSH', 'ZEC',
      'EOS', 'ETC', 'XRP'
    ],
    markets : [
